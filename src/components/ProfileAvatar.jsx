        ...
        >
          <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
            <div className="px-1 py-1 ">
              <Menu.Item>
                <h1 className="text-start px-3 mt-2 text-gray-600">
                  {value?.fullName || value?.username || value?.name}
                </h1>
              </Menu.Item>
              <Menu.Item>
                <h1 className="text-start px-3 text-sm font-semibold text-gray-500">
                  {value?.email}
                </h1>
              </Menu.Item>
            </div>
            <div className="px-1 py-1">
              <Menu.Item>
                {({ active }) => (
                  <button
                    className={`${
                      active ? 'bg-[#0f766e] text-white' : 'text-gray-900'
                    } group flex w-full items-center rounded-md px-2 py-2 text-sm`}
                  >
                    {active ? (
                      <VscAccount
                        className="mr-2 h-5 w-5 text-[#44ada5]"
                        aria-hidden="true"
                      />
                    ) : (
                      <VscAccount
